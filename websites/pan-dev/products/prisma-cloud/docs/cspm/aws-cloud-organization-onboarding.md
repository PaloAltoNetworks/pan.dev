...
> **Recommended**: Use *Terraform* to onboard AWS cloud accounts. Refer to
[AWS organization onboarding](https://registry.terraform.io/providers/PaloAltoNetworks/prismacloud/latest/docs/resources/org_cloud_account_v2) guide.
...
​
  ...
  def wait_till_stack_created(cloud_formation_client, stack_name, state):
      ...
      retry_count = 0
      max_retry_count = 6  # Increase If stack Creation is taking more time
      status = None
  ...
  ​
  ​
  # Intialize boto3 client, Provide aws access key and secret access key for the Organization to be onboarded
  ...
  if response.status_code == 200:
      print("Successfully onboarded organization")
  else:
      print(f"failed to Onboard organization. status_code: {response.status_code}")
      print(f"failed to Onboard organization. response: {response.text}")
      print(f"failed to Onboard organization. status: {response.headers.get('x-redlock-status')}")
  ```
</details>
