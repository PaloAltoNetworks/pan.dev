...
Expedition API responses are in JSON format. In the case of a login API call, in the content section we will obtain an API key (`apikey`) and a CSRF Token (`csrfToken`).
...
| ------ | ----------------------------------------------- | --------------------------------- | ------------------------------------------------------------------ |
| POST   | <small>`https://localhost/api/v1/login`</small> | <small>username, password</small> | <small>{**"username"**:"admin", **"password"**:"paloalto"}</small> |
