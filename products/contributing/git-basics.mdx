...
- **`git pull`** pulls changes from the repository you cloned and merges them into their respective branches
...
- **`git fetch`** reaches out to the repository you cloned to ask if there are changes, does not merge them

  ```bash
  git fetch
  ```

  Use this to see if there are changes without merging them
...
- **`git merge`** merges `branch-name` into your working branch
  ...
  _Caution: this could result in a conflict._
...
- **`git add <filename>`** adds a file you've edited to be staged for a commit
  ...
  ```bash
  git add products/contributing/git-basics.mdx
  ```
