...
State snapshots store information about the state of a particular device area. Some snapshots can take optional configurations to customize the snapshot behavior otherwise they store every possible information about an area. You may also use reports or custom code to extract a subset of information if required.
...
- `str` - the element simply specifies the name of a snapshot to run.
- `dict` - the element contains the snapshot name and a configuration (if a particular snapshot supports one); the dictionary format is as follows:
...
- values are dictionaries representing [`SnapResult`](/panos/docs/panos-upgrade-assurance/api/utils#class-snapresult) objects, containing four keys:

  - `state`: a `bool` representation of the snapshot outcome:
    - `True` when the snapshot was captured successfully,
    - `False` when an error occurred during snapshot capture.
  - `status`: a `str` representation of the [`SnapStatus`](/panos/docs/panos-upgrade-assurance/api/utils#class-snapstatus) enum:
    - `"SUCCESS"` when the snapshot was captured successfully,
    - `"ERROR"` when an error occurred during snapshot capture.
  - `reason`: a `str` providing explanation of the snapshot outcome, particularly useful for understanding error conditions.
  - `snapshot`: a `dict` or `None` containing the actual snapshot data when successful, `None` when an error occurred. The structure of this dictionary depends on the nature of data being captured (see sections below for details on each snapshot type).
...
When a snapshot fails to capture, the structure reflects the error:
...
result = {
  'routes': {
    'state': False,
    'status': 'ERROR',
    'reason': 'Command failed with error: Invalid command',
    'snapshot': None
  }
}
```
...
In this case:
- `state` is `False` (Python) / `false` (YAML) indicating failure
- `status` is `"ERROR"`
- `reason` provides the error details
- `snapshot` is `None` (Python) / `null` (YAML) since no data was captured
...
**Method:** [`CheckFirewall.get_arp_table_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_arp_table_snapshot)

### `content_version`

Grabs the currently installed Content DB version.

**Method:** [`CheckFirewall.get_content_db_version_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_content_db_version_snapshot)

### `ip_sec_tunnels`

Takes a snapshot of configuration of all IPSec tunnels along with their state.

**Method:** [`CheckFirewall.get_ip_sec_tunnels_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_ip_sec_tunnels_snapshot)

### `license`

Takes a snapshot of information about all licenses installed on a device.

**Method:** [`CheckFirewall.get_license_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_license_snapshot)

### `nics`

Takes a snapshot of a state of all configured (not installed) network interfaces.

**Method:** [`CheckFirewall.get_nics_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_nics_snapshot)

### `routes`

Takes a snapshot of the Route Table (this includes routes populated from DHCP as well as manually entered ones).

**Method:** [`CheckFirewall.get_routes_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_routes_snapshot)

### `bgp_peers`

Takes a snapshot of configuration of BGP peers along with their status.

**Method:** [`CheckFirewall.get_bgp_peers_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_bgp_peers_snapshot)

### `fib_routes`

Takes a snapshot of the Forwarding table (routes that are actually taken into forwarding decisions based on routing table).

**Method:** [`CheckFirewall.get_fib_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_fib_snapshot)

### `session_stats`

Gets information about the session statistics, such as current sessions count per a session type (TCP, UDP, etc).

**Method:** [`CheckFirewall.get_session_stats_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_session_stats_snapshot)

### `mtu`

Takes a snapshot of MTU sizes for all interfaces on the device.
...
**Method:** [`CheckFirewall.get_interfaces_mtu_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_interfaces_mtu_snapshot)

**Configuration parameters**

parameter | description
--- | ---
`include_subinterfaces` | (optional, defaults to `False`) Whether to include sub-interfaces in the results.
...
Takes a snapshot of Advanced Routing Engine (ARE) routing table entries. This snapshot is specifically for devices using the Advanced Routing Engine.

**Method:** [`CheckFirewall.get_are_routes_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_are_routes_snapshot)

### `are_fib_routes`

Takes a snapshot of Advanced Routing Engine (ARE) FIB (Forwarding Information Base) table entries. This snapshot is specifically for devices using the Advanced Routing Engine.

**Method:** [`CheckFirewall.get_are_fib_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_are_fib_snapshot)
...
reports = [
    ...
    {'are_routes': {
        'properties': ['!uptime', '!internalNextHopNum'],
        'count_change_threshold': 10
    }},
    {'are_fib_routes': {
        'properties': ['!flags'],
...
For most reports, a generic comparison method is used ([`SnapshotCompare.compare_type_generic()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_generic)). It produces the standardized dictionary. For details, see documentation for this method. Common method assigns a report result to a report area providing a dictionary where keys are report areas and values are report results.
...
**Method:** [`SnapshotCompare.compare_type_generic()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_generic)
...
Provides a report on differences between Advanced Routing Engine (ARE) routing table entries. This snapshot type is specifically designed for devices using the Advanced Routing Engine. It includes:

- availability of a route in one of the snapshots,
- for routes available in two snapshots, difference in route properties, such as protocol, nexthops, installed status, etc.

The comparison automatically:
- Filters to only installed routes for each destination per logical router
- Normalizes nexthop information for accurate comparison
- Organizes nexthops as interface/IP pairs

**Method:** [`SnapshotCompare.compare_type_are_routes()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_are_routes)

**Configuration parameters**

parameter | description
--- | ---
`properties` | (optional) a set of properties to skip or include when comparing two route entries, all properties are checked when this parameter is skipped
`count_change_threshold` | (optional) maximum difference percentage of changed entries in routing table in both snapshots, skipped when this property is not specified

**Sample configuration**

The following configuration:

- compares ARE routing table entries between both snapshots, but when comparing two entries the `uptime` and `internalNextHopNum` parameters are not taken into consideration,
- calculates the count of changed routing table entries from both snapshots and marks comparison as failed if the difference is bigger than 10%.
...
Provides a report on differences between Advanced Routing Engine (ARE) FIB (Forwarding Information Base) table entries. This snapshot type is specifically designed for devices using the Advanced Routing Engine. It includes:

- availability of a FIB entry in one of the snapshots,
- for FIB entries available in two snapshots, difference in FIB entry properties, such as destination, interface, nexthop, MTU, etc.
...
--- | ---
`properties` | (optional) a set of properties to skip or include when comparing two FIB entries, all properties are checked when this parameter is skipped
`count_change_threshold` | (optional) maximum difference percentage of changed entries in FIB table in both snapshots, skipped when this property is not specified

**Sample configuration**

The following configuration:

- compares ARE FIB table entries between both snapshots, but when comparing two entries the `flags` parameter is not taken into consideration,
- calculates the count of changed FIB table entries from both snapshots and marks comparison as failed if the difference is bigger than 10%.
...
This report is slightly different than reports made with the [`SnapshotCompare.compare_type_generic()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_generic) method as the snapshot data is different (refer to the [`CheckFirewall.get_session_stats_snapshot()`](/panos/docs/panos-upgrade-assurance/api/check_firewall#checkfirewallget_session_stats_snapshot) method documentation for details).
...
**Method:** [`SnapshotCompare.compare_type_metric_values()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_metric_values)
...
This report produces a `dict` as documented in the [`SnapshotCompare.compare_type_metric_values()`](/panos/docs/panos-upgrade-assurance/api/snapshot_compare#snapshotcomparecompare_type_metric_values) method documentation.
...
Provides a report on differences between interface MTU sizes. It simply compares MTU sizes for all interfaces from both snapshots.
