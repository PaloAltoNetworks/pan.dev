...
- You can use the [SSO user creation API](/sase/api/iam/create-a-user-account)
...
[SSO user verification API](/sase/api/iam/verify-a-user-account).

## Access Policies

As described in [Assign Roles](/sase/docs/roles-assign), you grant a user account access to
SASE products by [applying an access policy](/sase/api/iam/assign-an-access-policy)
